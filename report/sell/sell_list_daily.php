...
$tomor = date('Y-m-d', strtotime($today." +1 days"));
$datethai = DateThai($today);
$num = 1;

$sql_t = 'SELECT SUM(P_Price) '
        . 'FROM t_sell ts JOIN t_sell_detail tsd ON ts.ORD_ID = tsd.ORD_ID '
        . 'JOIN t_employee te ON ts.Emp_ID = te.Emp_ID '
        . 'WHERE (Date_Sell >= "' . $today . '" AND Date_Sell <= "' . $tomor . '")';

$stmt_t = $dbh->prepare($sql_t);
$stmt_t->execute();
$result_t = $stmt_t->fetch();
$sell_total = number_format($result_t['SUM(P_Price)']) . ' บาท';
...
$header = '<div style="text-align:center"><img src="../../img/onelogo.png" alt="" width="100px"></div>'
        ...
        . '<table border="0"><tr><td align="left">รายงานขายประจำวันที่ : ' . $datethai . ' </td><td align="right">ยอดขายรวม : ' . $sell_total . ' </td></tr></table>';
...
$mpdf->Output('SellDailyReport.pdf', 'I');
$dbh = null;
?>